...
import {
  ...
  scanEvents,
  activityLogs,
  assertAutomotiveTransition,
  getAutomotiveTimestampFieldForStatus,
  type Material,
  type Hall,
  type Station,
  type Stand,
...
function prepareUserResponse<T extends { password?: string; role?: string }>(
  user: T,
): Omit<T, "password"> {
...
function getTodayBerlin(): Date {
  const berlinDateStr = new Date().toLocaleString("en-US", {
    ...
    if (stand) {
      ...
      if (stand.stationId) {
        ...
        const [station] = await db
          .select()
          .from(stations)
          .where(eq(stations.id, stand.stationId));
        if (station?.hallId) {
...
async function generateDailyTasksScheduled() {
  try {
    ...
    const openDailyTasks = await db
      .select()
      .from(tasks)
      .where(and(eq(tasks.taskType, "DAILY_FULL"), eq(tasks.status, "OPEN")));
    ...
    for (const task of openDailyTasks) {
      if (task.dedupKey && !task.dedupKey.endsWith(`:${todayStr}`)) {
        ...
        const standMeta = task.standId
          ? await buildStandContextMeta(task.standId)
          : {};
        await createAuditEvent({
          ...
          afterData: {
            status: "CANCELLED",
            reason: "Auto-cancelled: New daily task generated",
          },
          metaJson: {
            ...standMeta,
    ...
    if (cancelledCount > 0) {
      console.log(
        `[DailyTaskScheduler] Auto-cancelled ${cancelledCount} previous OPEN daily tasks.`,
      );
    }

    // Get dailyFull stands
    const dailyFullStands = await db
      .select()
      .from(stands)
      .where(and(eq(stands.dailyFull, true), eq(stands.isActive, true)));
    ...
    for (const stand of dailyFullStands) {
      ...
      try {
        ...
        console.error(
          `[DailyTaskScheduler] Failed to create task for stand ${stand.id}:`,
          e,
        );
      }
    }
    console.log(
      `[DailyTaskScheduler] Completed. Created: ${createdCount}, Skipped (duplicates): ${skippedCount}`,
    );
  } catch (error) {
  ...
  try {
    ...
    for (const schedule of activeSchedules) {
      ...
      const [stand] = await db
        .select()
        .from(stands)
        .where(eq(stands.id, schedule.standId));
      if (!stand || !stand.isActive) {
        console.log(
          `[FlexibleScheduler] Stand ${schedule.standId} not found or inactive, skipping schedule ${schedule.id}`,
        );
        continue;
      }
      ...
      for (let dayOffset = 0; dayOffset <= createDaysAhead; dayOffset++) {
        ...
        const [hours, minutes] = (schedule.timeLocal || "06:00")
          .split(":")
          .map(Number);
        ...
        try {
          ...
          console.error(
            `[FlexibleScheduler] Failed to create task for schedule ${schedule.id}, date ${dateStr}:`,
            e,
          );
          totalErrors++;
        }
    ...
    console.log(
      `[FlexibleScheduler] Completed. Created: ${totalCreated}, Skipped (duplicates): ${totalSkipped}, Errors: ${totalErrors}`,
    );
  } catch (error) {
...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    async (req, res) => {
      try {
        ...
        if (!["DAILY", "WEEKLY", "INTERVAL"].includes(ruleType)) {
          return res
            .status(400)
            .json({ error: "ruleType must be DAILY, WEEKLY, or INTERVAL" });
        }
        ...
        if (
          ...
          return res
            .status(400)
            .json({ error: "weekdays is required for WEEKLY rule type" });
        }
        ...
        const [newSchedule] = await db
          ...
          .values({
            ...
            startDate:
              ruleType === "INTERVAL" && startDate ? new Date(startDate) : null,
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        if (createDaysAhead !== undefined)
          updateData.createDaysAhead = createDaysAhead;
    ...
    async (req, res) => {
      try {
        ...
        if (!schedule.schedule.isActive) {
          return res
            .status(400)
            .json({ error: "Cannot run inactive schedule" });
        }
        ...
        const [existingTask] = await db
          .select()
          .from(tasks)
          .where(eq(tasks.dedupKey, dedupKey));
        if (existingTask) {
        ...
        const standMeta = await buildStandContextMeta(
          schedule.schedule.standId,
        );
        await createAuditEvent({
          taskId: newTask.id,
        ...
        if (error?.code === "23505") {
          return res
            .status(409)
            .json({ error: "Task already exists for this schedule today" });
        }
  ...
  app.get(
    "/api/admin/schedules/:id/preview",
    requireAuth,
    requireAdmin,
    async (req, res) => {
      try {
        ...
        for (let dayOffset = 0; dayOffset < days; dayOffset++) {
          ...
          if (schedule.ruleType === "INTERVAL" && schedule.startDate) {
            const startDateLocal = getDateInTimezone(
              schedule.startDate,
              timezone,
            );
  ...
  app.post("/api/admin/tasks", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (!title || !standId) {
        return res
          .status(400)
          .json({ error: "Title and standId are required" });
      }
      ...
      if (scheduledFor) {
        ...
        if (!parsedDate || isNaN(parsedDate.getTime())) {
          return res
            .status(400)
            .json({ error: "Invalid scheduledFor date format" });
        }
  ...
  app.post(
    "/api/warehouse-containers/:id/empty",
    requireAuth,
    requireAdmin,
    async (req, res) => {
      try {
        ...
        if (!container) {
          return res
            .status(404)
            .json({ error: "Warehouse container not found" });
        }
        ...
        console.error(
          "[WarehouseContainers] Failed to empty container:",
          error,
        );
  ...
  app.post("/api/customers", requireAuth, requireAdmin, async (req, res) => {
    try {
      const { name, address, contactName, contactPhone, contactEmail, notes } =
        req.body;
  ...
  app.post(
    "/api/containers/customer/:id/regenerate-qr",
    requireAuth,
    requireAdmin,
    async (req, res) => {
      try {
        ...
        const existingContainer = await storage.getCustomerContainer(
          req.params.id,
        );
        ...
        if (!container) {
          return res
            .status(500)
            .json({ error: "Failed to regenerate QR code" });
        }
  ...
  app.get("/api/containers/warehouse/qr/:qrCode", async (req, res) => {
    try {
      // First try by QR code, then by container ID
      let container = await storage.getWarehouseContainerByQR(
        req.params.qrCode,
      );
      if (!container) {
  ...
  app.patch("/api/containers/warehouse/:id", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "Failed to update container", details: String(error) });
    }
  });

  // Admin-only: Regenerate QR code for warehouse container
  app.post(
    "/api/containers/warehouse/:id/regenerate-qr",
    requireAuth,
    requireAdmin,
    async (req, res) => {
      try {
        ...
        const existingContainer = await storage.getWarehouseContainer(
          req.params.id,
        );
        if (!existingContainer) {
          return res.status(404).json({ error: "Container not found" });
  ...
  app.post(
    "/api/containers/warehouse/:id/reset",
    requireAuth,
    async (req, res) => {
      try {
        ...
        if (!authUser || (userRole !== "admin" && userRole !== "driver")) {
          return res
            .status(403)
            .json({ error: "Only admin or driver roles can empty containers" });
        }
  ...
  app.post("/api/tasks", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (!materialType && req.body.containerID) {
        const container = await storage.getCustomerContainer(
      ...
      if (taskData.scheduledTime) {
        ...
        if (isNaN(parsedDate.getTime())) {
          return res
            .status(400)
            .json({ error: "Invalid scheduledTime format" });
        }
      ...
      if (taskData.deliveryContainerID && taskData.plannedQuantity) {
        const targetContainer = await storage.getWarehouseContainer(
          taskData.deliveryContainerID,
        );
        if (!targetContainer) {
          return res
            .status(400)
            .json({ error: "Zielcontainer nicht gefunden" });
        }

        const remainingCapacity =
          targetContainer.maxCapacity - targetContainer.currentAmount;
        if (taskData.plannedQuantity > remainingCapacity) {
          return res.status(400).json({
            error:
              "Zielcontainer hat nicht genug übriges Volumen für diese Menge.",
            remainingCapacity,
  ...
  app.delete("/api/tasks/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (!deleted) {
        return res
          .status(500)
          .json({ error: "Fehler beim Löschen des Auftrags" });
      }
    ...
    try {
      ...
      if (!isAdmin && !isAssignedDriver && task.assignedTo) {
        return res.status(403).json({
          error:
            "Nur der zugewiesene Fahrer oder ein Admin kann diesen Auftrag annehmen.",
          assignedTo: task.assignedTo,
        });
      }
      ...
      if (LATER_STATES.includes(task.status)) {
        ...
        const sourceContainer = await storage.getCustomerContainer(
          task.containerID,
        );
        let targetContainer = null;
        if (task.deliveryContainerID) {
          targetContainer = await storage.getWarehouseContainer(
            task.deliveryContainerID,
          );
        }
          ...
          sourceContainer: sourceContainer
            ? {
                ...
                plannedPickupQuantity:
                  task.plannedQuantity || task.estimatedAmount || 0,
              }
            : null,
        };
      ...
      if (!sourceContainer) {
        return res
          .status(404)
          .json({ error: "Kundencontainer nicht gefunden" });
      }
      ...
      if (task.deliveryContainerID) {
        ...
        if (targetContainer) {
          ...
          if (sourceContainer.materialType !== targetContainer.materialType) {
            return res.status(400).json({
              error:
                "Der Zielcontainer enthält ein anderes Material. Bitte wähle einen passenden Lagercontainer.",
          ...
          if (
            ...
            task.plannedQuantity > remainingCapacity
          ) {
            return res.status(400).json({
              error:
                "Zielcontainer hat nicht genug übriges Volumen für diese Menge.",
      ...
      const response: any = {
        ...
        sourceContainer: {
          ...
          plannedPickupQuantity:
            updatedTask.plannedQuantity || updatedTask.estimatedAmount || 0,
        },
      };
  ...
  app.post("/api/tasks/:id/pickup", async (req, res) => {
    try {
      ...
      if (!isAdmin && !isAssignedDriver) {
        return res.status(403).json({
          error:
            "Nur der zugewiesene Fahrer oder ein Admin kann diesen Auftrag abholen.",
          assignedTo: task.assignedTo,
        });
      }
      ...
      if (task.status !== "ACCEPTED") {
        return res.status(400).json({
          error:
            "Auftrag muss zuerst angenommen werden bevor er abgeholt werden kann",
  ...
  app.post("/api/tasks/:id/delivery", async (req, res) => {
    try {
      const { userId, warehouseContainerId, amount, location, geoLocation } =
        req.body;
      const task = await storage.getTask(req.params.id);
      ...
      if (!isAdmin && !isAssignedDriver) {
        return res.status(403).json({
          error:
            "Nur der zugewiesene Fahrer oder ein Admin kann diesen Auftrag abliefern.",
          assignedTo: task.assignedTo,
        });
      }
      ...
      const deliveredAmount =
        amount || task.plannedQuantity || task.estimatedAmount || 0;
      ...
      let updatedTask = await storage.updateTaskStatus(
        req.params.id,
        "DELIVERED",
      );
      if (!updatedTask) {
      ...
      if (!updatedTask) {
        return res
          .status(400)
          .json({ error: "Fehler beim Abschließen des Auftrags" });
      }
    ...
    try {
      ...
      if (task.claimedByUserId) {
        if (!isClaimExpired(task.claimedAt)) {
          ...
          return res.status(409).json({
            error:
              "Auftrag wurde bereits von einem anderen Benutzer beansprucht",
            claimedBy: claimingUser?.name || "Unbekannt",
            claimedAt: task.claimedAt,
            expiresAt: task.claimedAt
              ? new Date(
                  task.claimedAt.getTime() + CLAIM_TTL_MINUTES * 60 * 1000,
                )
              : null,
          });
        } else {
      ...
      await createAuditEvent({
        ...
        entityId: task.id,
        beforeData: {
          claimedByUserId: autoReleasedExpired ? null : task.claimedByUserId,
        },
  ...
  app.post("/api/tasks/:id/transition", requireAuth, async (req, res) => {
    try {
      const { toStatus, weightKg, targetWarehouseContainerId, reason } =
        req.body;
      const authUser = (req as any).authUser;
      ...
      if (task.status === "TAKEN_OVER" && toStatus === "WEIGHED") {
        if (weightKg === undefined || weightKg === null) {
          return res
            .status(400)
            .json({ error: "weightKg is required for WEIGHED status" });
        }
      }
      ...
      const eventMetaJson = {
        ...
        targetWarehouseContainerId:
          updateData.targetWarehouseContainerId || undefined,
        autoClaimed,
        autoReleased,
      };
      ...
      if (autoReleased) {
        await createAuditEvent({
          ...
          entityId: task.id,
          beforeData: {
            claimedByUserId: autoClaimed ? authUser.id : task.claimedByUserId,
          },
          afterData: { claimedByUserId: null, claimedAt: null },
          metaJson: {
            ...standMeta,
  ...
  app.post("/api/tasks/:id/handover", requireAuth, async (req, res) => {
    try {
      ...
      const isCurrentOwner =
        task.claimedByUserId === authUser.id || task.assignedTo === authUser.id;

      if (!isAdmin && !isCurrentOwner) {
        return res.status(403).json({
          error:
            "Nur der aktuelle Besitzer oder ein Admin kann diesen Auftrag übergeben",
        });
      }
      ...
      if (NON_TRANSFERABLE_STATUSES.includes(task.status)) {
        return res.status(400).json({
          error:
            "Abgeschlossene oder stornierte Aufträge können nicht übergeben werden",
          currentStatus: task.status,
        });
      }

      const oldUser = task.claimedByUserId
        ? await storage.getUser(task.claimedByUserId)
        : authUser;
  ...
  app.get("/api/scan-events", async (req, res) => {
    try {
      ...
      const events = await storage.getScanEvents(
        Object.keys(filters).length > 0 ? filters : undefined,
      );
      res.json(events);
    } catch (error) {
    ...
    try {
      ...
      if (scanContext === "TASK_COMPLETE_AT_WAREHOUSE") {
        ...
        if (isNaN(weight) || weight <= 0) {
          return res
            .status(400)
            .json({ error: "measuredWeight muss größer als 0 sein" });
        }
        ...
        const warehouseContainer =
          await storage.getWarehouseContainer(containerId);
        if (!warehouseContainer) {
          return res
            .status(404)
            .json({ error: "Lagercontainer nicht gefunden" });
        }
        ...
        const scanEvent = await storage.createScanEvent({
          ...
          locationType,
          locationDetails:
            locationDetails || warehouseContainer.warehouseZone || null,
          geoLocation: geoLocation || null,
          scanResult: "SUCCESS",
          resultMessage: null,
  ...
  app.get("/api/activity-logs", async (req, res) => {
    try {
      const { userId, containerId, type, taskId, startDate, endDate } =
        req.query;
      const filters: {
        userId?: string;
        containerId?: string;
  ...
  app.get("/api/activity-logs/export/csv", async (req, res) => {
    try {
      ...
      const csvHeader =
        "ID,Datum,Uhrzeit,Benutzer,Typ,Nachricht,Container ID,Auftrag ID\n";
      const csvRows = logs
        .map((log) => {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=aktivitaetslog-${new Date().toISOString().split("T")[0]}.csv`,
      );
      res.send("\uFEFF" + csv);
    } catch (error) {
    ...
    try {
      ...
      const driverStats = drivers.map((driver) => {
        ...
        return {
          ...
          inProgress: driverTasks.filter((t) =>
    ...
    try {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const totalDelivered = dayTasks.reduce((sum, t) => {
          const container = warehouseContainers.find(
      ...
      const materialBreakdown = warehouseContainers.reduce(
        (acc, c) => {
  ...
  app.get("/api/dashboard/stats", async (req, res) => {
    try {
      ...
      const openTasks = tasksToCount.filter((t) =>
        openStatuses.includes(t.status),
      ).length;
      const inProgressTasks = tasksToCount.filter((t) =>
        inProgressStatuses.includes(t.status),
      ).length;
      const completedTasks = tasksToCount.filter((t) =>
        completedStatuses.includes(t.status),
      ).length;
      const completedToday = todayTasks.filter((t) =>
        completedStatuses.includes(t.status),
      ).length;
      const cancelledTasks = tasksToCount.filter((t) =>
        cancelledStatuses.includes(t.status),
      ).length;
      const activeDrivers = users.filter(
        (u) => (u.role === "driver" || u.role === "DRIVER") && u.isActive,
      ).length;
      ...
      const totalCapacity = warehouseContainers.reduce(
        (acc, c) => acc + c.maxCapacity,
        0,
      );
      const usedCapacity = warehouseContainers.reduce(
  ...
  app.get("/api/drivers/:id/stats", async (req, res) => {
    try {
      ...
      const cancelledTasks = allTasks.filter((t) =>
        cancelledStatuses.includes(t.status),
      ).length;
      ...
      const lastActivity =
        activityLogs.length > 0 ? activityLogs[0].timestamp : null;
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const driverOverview = await Promise.all(
          drivers.map(async (driver) => {
            const driverTasks = allTasks.filter(
              (t) => t.assignedTo === driver.id,
            );
            ...
            return {
              ...
              openTasks: driverTasks.filter((t) =>
                openStatuses.includes(t.status),
              ).length,
              inProgressTasks: driverTasks.filter((t) =>
                inProgressStatuses.includes(t.status),
              ).length,
              completedTasks: driverTasks.filter((t) =>
                completedStatuses.includes(t.status),
              ).length,
              cancelledTasks: driverTasks.filter((t) =>
                cancelledStatuses.includes(t.status),
              ).length,
  ...
  app.get("/api/stations/:id/details", requireAuth, async (req, res) => {
    try {
      ...
      const [hall] = await db
        .select()
        .from(halls)
        .where(eq(halls.id, station.hallId));
      ...
      const materialIds = stationStands
        .map((s) => s.materialId)
      ...
      const standsWithDetails = stationStands.map((stand) => ({
        ...stand,
        material: stand.materialId
          ? materialMap.get(stand.materialId) || null
          : null,
  ...
  app.post("/api/stations", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (!hallId || !name || !code) {
        return res
          .status(400)
          .json({ error: "hallId, name, and code are required" });
      }
  ...
  app.put("/api/stations/:id", requireAuth, requireAdmin, async (req, res) => {
    try {
      const { name, code, sequence, locationMeta, positionMeta, isActive } =
        req.body;
      ...
      if (code && code !== existingStation.code) {
        const [duplicateStation] = await db
          .select()
          .from(stations)
          .where(
            and(
    ...
    async (req, res) => {
      try {
        ...
        if (hallId === existingStation.hallId) {
          return res
            .status(400)
            .json({ error: "Station ist bereits in dieser Halle" });
        }
        ...
        const [duplicateStation] = await db
          ...
          .where(
            and(
              eq(stations.hallId, hallId),
              eq(stations.code, existingStation.code),
            ),
          );
        if (duplicateStation) {
          return res.status(400).json({
        ...
        const [sourceHall] = await db
          .select()
          .from(halls)
          .where(eq(halls.id, existingStation.hallId));
    ...
    try {
      ...
      if (materialId !== undefined && materialId !== existingStand.materialId) {
        if (materialId !== null) {
          ...
          changes.push(
            `Material: ${existingStand.materialId || "keins"} → ${materialId}`,
          );
        } else {
      ...
      if (stationId !== undefined && stationId !== existingStand.stationId) {
        ...
        if (!targetStation.isActive) {
          return res
            .status(400)
            .json({ error: "Ziel-Station ist nicht aktiv" });
        }
        const [targetHall] = await db
          .select()
          .from(halls)
          .where(eq(halls.id, targetStation.hallId));
        if (!targetHall?.isActive) {
          return res
            .status(400)
            .json({ error: "Halle der Ziel-Station ist nicht aktiv" });
        }
      ...
      if (isActive !== undefined && isActive !== existingStand.isActive) {
        ...
        if (!isActive && existingStand.isActive) {
          ...
          if (standBoxes.length > 0) {
            ...
            changes.push(
              `${standBoxes.length} Box(en) vom Stellplatz abgemeldet`,
            );
          }
        }
      }
      ...
      res
        .status(500)
        .json({ error: "Fehler beim Bearbeiten des Stellplatzes" });
    }
  });
  ...
  app.get("/api/stands/qr/:qrCode", async (req, res) => {
    try {
      const [stand] = await db
        .select()
        .from(stands)
        .where(eq(stands.qrCode, req.params.qrCode));
      if (!stand) {
        return res.status(404).json({ error: "Stand not found" });
      }
      ...
      const [station] = await db
        ...
        .where(eq(stations.id, stand.stationId));
      const [hall] = station
        ? await db.select().from(halls).where(eq(halls.id, station.hallId))
        : [null];
      // Get material info
      const [material] = stand.materialId
        ? await db
            .select()
            .from(materials)
            .where(eq(materials.id, stand.materialId))
        : [null];
      // Get boxes at this stand
      const standBoxes = await db
        .select()
        .from(boxes)
        .where(eq(boxes.standId, stand.id));
  ...
  app.post("/api/stands", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (!stationId || !identifier || !qrCode) {
        return res
          .status(400)
          .json({ error: "stationId, identifier, and qrCode are required" });
      }
  ...
  app.get(
    "/api/admin/stands/:standId/boxes",
    requireAuth,
    requireAdmin,
    async (req, res) => {
      try {
        ...
        const result = await db
          .select()
          .from(boxes)
          .where(and(eq(boxes.standId, standId), eq(boxes.isActive, true)));
    ...
    async (req, res) => {
      try {
        ...
        if (!boxId && !boxSerial && !boxQr) {
          return res
            .status(400)
            .json({ error: "boxId, boxSerial oder boxQr erforderlich" });
        }
        ...
        if (boxId) {
          ...
          [box] = await db
            .select()
            .from(boxes)
            .where(eq(boxes.serial, boxSerial));
        } else if (boxQr) {
        ...
        if (box.standId !== null && box.standId !== standId) {
          const [currentStand] = await db
            .select()
            .from(stands)
            .where(eq(stands.id, box.standId));
          return res.status(409).json({
    ...
    async (req, res) => {
      try {
        ...
        if (box.standId !== standId) {
          return res
            .status(400)
            .json({ error: "Box ist nicht an diesem Stellplatz" });
        }
  ...
  app.get("/api/boxes/qr/:qrCode", async (req, res) => {
    try {
      const [box] = await db
        .select()
        .from(boxes)
        .where(eq(boxes.qrCode, req.params.qrCode));
      if (!box) {
  ...
  app.post("/api/boxes", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (!qrCode || !serial) {
        return res
          .status(400)
          .json({ error: "qrCode and serial are required" });
      }
  ...
  app.get("/api/automotive/tasks", requireAuth, async (req, res) => {
    try {
      const { status, materialId, stationId, hallId, from, to, scheduledFor } =
        req.query;
  ...
  app.post("/api/automotive/tasks", requireAuth, async (req, res) => {
    try {
      ...
      if (!boxId || !standId) {
        return res
          .status(400)
          .json({ error: "boxId and standId are required" });
      }
      ...
      if (taskType && !validTaskTypes.includes(taskType)) {
        return res
          .status(400)
          .json({ error: "taskType must be MANUAL or DAILY_FULL" });
      }
  ...
  app.put("/api/automotive/tasks/:id/status", requireAuth, async (req, res) => {
    try {
      ...
      if (task.status === "OPEN" && status === "PICKED_UP" && scannedBoxId) {
        const [scannedBox] = await db
          .select()
          .from(boxes)
          .where(eq(boxes.id, scannedBoxId));
        if (!scannedBox) {
          return res
            .status(404)
            .json({ error: "Gescannte Box nicht gefunden" });
        }
        ...
        if (task.standId && scannedBox.standId !== task.standId) {
          return res
            .status(400)
            .json({ error: "Box befindet sich nicht am richtigen Stand" });
        }
      }
      ...
      if (task.status === "IN_TRANSIT" && status === "DROPPED_OFF") {
        if (!scannedStandId) {
          return res
            .status(400)
            .json({ error: "Stand-Scan erforderlich für Abgabe" });
        }
        // Verify stand exists
        const [scannedStand] = await db
          .select()
          .from(stands)
          .where(eq(stands.id, scannedStandId));
        if (!scannedStand) {
          return res
            .status(404)
            .json({ error: "Gescannter Stand nicht gefunden" });
        }
      }
    ...
    try {
      ...
      if (task.standId) {
        const [stand] = await db
          .select()
          .from(stands)
          .where(eq(stands.id, task.standId));
      ...
      const containers = await db
        ...
        .where(
          and(
            ...
            materialId
              ? eq(warehouseContainers.materialId, materialId)
              : isNull(warehouseContainers.materialId),
          ),
        );
  ...
  app.post(
    "/api/automotive/daily-tasks/generate",
    requireAuth,
    requireAdmin,
    async (req, res) => {
      try {
        ...
        for (const stand of dailyFullStands) {
          ...
          try {
            ...
            if (e?.code === "23505") {
              skipped.push({
                standId: stand.id,
                reason: "Task already exists for today (dedupKey)",
              });
              continue;
            }
            throw e;
  ...
  app.get("/api/automotive/daily-tasks/status", async (req, res) => {
    try {
      ...
      const status = dailyFullStands.map((stand) => ({
        ...
        generatedToday: stand.lastDailyTaskGeneratedAt
          ? stand.lastDailyTaskGeneratedAt >= todayStart
          : false,
      }));
  ...
  app.get("/api/daily-tasks/today", async (req, res) => {
    try {
      ...
      const filteredTasks = todayTasks.filter(
        (t) =>
          t.dedupKey?.startsWith(`DAILY:`) &&
          t.dedupKey?.endsWith(`:${todayStr}`),
      );
  ...
  app.post("/api/scan/place-box", requireAuth, async (req, res) => {
    try {
      ...
      if (!standQr || !boxQr) {
        return res
          .status(400)
          .json({ error: "standQr and boxQr are required" });
      }
      ...
      const [stand] = await db
        .select()
        .from(stands)
        .where(eq(stands.qrCode, standQr));
      if (!stand) {
        return res.status(404).json({ error: "Stellplatz nicht gefunden" });
      }
      ...
      const existingBoxes = await db
        .select()
        .from(boxes)
        .where(and(eq(boxes.standId, stand.id), eq(boxes.status, "AT_STAND")));
      ...
      if (box.currentTaskId) {
        const [task] = await db
          .select()
          .from(tasks)
          .where(eq(tasks.id, box.currentTaskId));
        if (
          task &&
          ...
          try {
            ...
            const timestampField =
              getAutomotiveTimestampFieldForStatus("DROPPED_OFF");
            ...
            console.log(
              `[PlaceBox] Task ${task.id} transition to DROPPED_OFF not allowed from ${task.status}`,
            );
          }
        }
      }
    ...
    try {
      ...
      if (previousStandId) {
        const [stand] = await db
          .select()
          .from(stands)
          .where(eq(stands.id, previousStandId));
        previousStand = stand;
        if (stand) {
          const [station] = await db
            .select()
      ...
      const [scanEvent] = await db
        ...
        .values({
          ...
          locationDetails:
            locationDetails ||
            (previousStand
              ? `Abgeholt von ${previousStand.identifier}`
              : "Abgeholt"),
          geoLocation: geo || null,
          scanResult: "SUCCESS",
      ...
      // Build context meta
      const contextMeta = previousStandId
        ? await buildStandContextMeta(previousStandId)
        : { boxId: box.id };
        ...
        if (task) {
          ...
          if (newStatus) {
            try {
              ...
              const timestampField =
                getAutomotiveTimestampFieldForStatus(newStatus);
              ...
              console.log(
                `[PickupBox] Task ${task.id} transition to ${newStatus} not allowed from ${task.status}`,
              );
            }
          }
        }
  ...
  app.get("/api/task-events", async (req, res) => {
    try {
      ...
      if (!taskId || typeof taskId !== "string") {
        return res
          .status(400)
          .json({ error: "taskId query parameter is required" });
      }
  ...
  app.get("/api/activity", async (req, res) => {
    try {
      ...
      if (materialId && typeof materialId === "string") {
        conditions.push(
          sql`${taskEvents.metaJson}->>'materialId' = ${materialId}`,
        );
      }
      if (stationId && typeof stationId === "string") {
        conditions.push(
          sql`${taskEvents.metaJson}->>'stationId' = ${stationId}`,
        );
      }
      if (hallId && typeof hallId === "string") {
      ...
      if (groupBy === "material") {
        const result = await db
          .select({
            ...
            taskCount: count(),
            avgLeadTimeMinutes: avg(
              sql`EXTRACT(EPOCH FROM (${tasks.disposedAt} - ${tasks.createdAt})) / 60`,
            ),
          })
          .from(tasks)
          .leftJoin(materials, eq(tasks.materialType, materials.id))
  ...
  app.get("/api/analytics/backlog", requireAuth, async (req, res) => {
    try {
      ...
      const tasksWithHours = result.map((task) => ({
        ...task,
        hoursInStatus: task.updatedAt
          ? Math.round(
              (now - new Date(task.updatedAt).getTime()) / (1000 * 60 * 60),
            )
          : null,
      }));
    ...
    async (req, res) => {
      try {
        ...
        const result = await db
          ...
          .where(
            and(
              eq(stands.stationId, stationId as string),
  ...
  app.post("/api/admin/tasks", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (station.hallId !== hallId) {
        return res
          .status(400)
          .json({ error: "Station does not belong to the specified hall" });
      }
      ...
      if (stand.stationId !== stationId) {
        return res
          .status(400)
          .json({ error: "Stand does not belong to the specified station" });
      }
      if (!stand.isActive) {
      ...
      console.log(
        `[Admin] Manual task created: ${newTask.id} for stand ${stand.identifier}`,
      );
      res.status(201).json(newTask);
    } catch (error) {
    ...
    async (req, res) => {
      try {
        ...
        const stationsData = [
          {
            code: "K13-S01",
            hallCode: "K13",
            name: "Cluster 338-340",
            x: 0.11,
            y: 0.46,
            label: "Cluster 338-340 (links)",
          },
          {
            code: "K18-S01",
            hallCode: "K18",
            name: "Cluster 712/623/615",
            x: 0.54,
            y: 0.22,
            label: "Cluster 712/623/615 (oben)",
          },
          {
            code: "K18-S02",
            hallCode: "K18",
            name: "Cluster 646/647/653",
            x: 0.76,
            y: 0.52,
            label: "Cluster 646/647/653 (rechts)",
          },
          {
            code: "K18-S03",
            hallCode: "K18",
            name: "Cluster 655/656",
            x: 0.79,
            y: 0.86,
            label: "Cluster 655/656 (unten rechts)",
          },
          {
            code: "K18-S04",
            hallCode: "K18",
            name: "Cluster 680-683",
            x: 0.32,
            y: 0.75,
            label: "Cluster 680-683 (links unten)",
          },
          {
            code: "K19-S01",
            hallCode: "K19",
            name: "Cluster 625-627",
            x: 0.21,
            y: 0.46,
            label: "Cluster 625-627 (links)",
          },
          {
            code: "K19-S02",
            hallCode: "K19",
            name: "Cluster 701/702/710/711",
            x: 0.36,
            y: 0.17,
            label: "Cluster 701/702/710/711 (oben links)",
          },
          {
            code: "K19-S03",
            hallCode: "K19",
            name: "Cluster 604/605/606/612",
            x: 0.67,
            y: 0.14,
            label: "Cluster 604/605/606/612 (oben rechts)",
          },
          {
            code: "K19-S04",
            hallCode: "K19",
            name: "Cluster 201/62/616/617",
            x: 0.49,
            y: 0.48,
            label: "Cluster 201/62/616/617 (Mitte)",
          },
          {
            code: "K19-S05",
            hallCode: "K19",
            name: "Cluster 629-633",
            x: 0.37,
            y: 0.86,
            label: "Cluster 629-633 (unten Mitte)",
          },
          {
            code: "K19-S06",
            hallCode: "K19",
            name: "Cluster 608-611/602/609/610",
            x: 0.71,
            y: 0.8,
            label: "Cluster 608-611/602/609/610 (unten rechts)",
          },
          {
            code: "K25-S01",
            hallCode: "K25",
            name: "Cluster 36/37/47",
            x: 0.1,
            y: 0.65,
            label: "Cluster 36/37/47 (links unten)",
          },
          {
            code: "K25-S02",
            hallCode: "K25",
            ...
            x: 0.19,
            y: 0.52,
            label: "Cluster 706 (links Mitte)",
          },
          {
            code: "K25-S03",
            hallCode: "K25",
            name: "Cluster 67/68/69/660",
            x: 0.53,
            y: 0.66,
            label: "Cluster 67/68/69/660 (unten Mitte)",
          },
          {
            code: "K25-S04",
            hallCode: "K25",
            name: "Cluster 225-228/224",
            x: 0.71,
            y: 0.62,
            label: "Cluster 225-228/224 (rechts)",
          },
          {
            code: "K25-S05",
            hallCode: "K25",
            ...
            x: 0.81,
            y: 0.17,
            label: "Cluster 41/663 (oben rechts)",
          },
          {
            code: "K25-S06",
            hallCode: "K25",
            name: "Cluster 672/714",
            x: 0.07,
            y: 0.35,
            label: "Cluster 672/714 (oben links)",
          },
        ];
        ...
        for (const s of stationsData) {
          ...
          if (!hallId) {
            console.warn(
              `[Seed] Hall ${s.hallCode} not found for station ${s.code}`,
            );
            continue;
          }
            ...
            if (existing) {
              ...
              try {
                ...
                if (err?.code === "23505") {
                  ...
                  console.error(
                    `[Seed] Failed to create stand ${fullId}:`,
                    err,
                  );
                }
              }
    ...
    async (req, res) => {
      try {
        ...
        if (x === undefined && y === undefined) {
          return res
            .status(400)
            .json({ error: "x oder y Koordinate erforderlich" });
        }

        if (x !== null && (typeof x !== "number" || x < 0 || x > 1)) {
          return res
            .status(400)
            .json({ error: "x muss eine Zahl zwischen 0 und 1 sein" });
        }
        if (y !== null && (typeof y !== "number" || y < 0 || y > 1)) {
          return res
            .status(400)
            .json({ error: "y muss eine Zahl zwischen 0 und 1 sein" });
        }
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Fehler beim Abrufen des Hallenmarkers" });
      }
    },
  );
   ...
   */
  app.patch(
    "/api/admin/stations/:id/position",
    requireAuth,
    requireAdmin,
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Fehler beim Setzen der Stationsposition" });
      }
    },
  );
      ...
      try {
        ...
        const hallsWithMarkers = hallsResult.map((h) => {
          ...
          return {
            ...
            mapMarker: locationMeta?.mapMarker || null,
            mapImageKey:
              locationMeta?.mapImageKey ||
              (h.code === "OUT" ? "OUT.png" : `${h.code}.png`),
          };
        });
        ...
        const stationsWithPositions = stationsResult.map((s) => {
          ...
          return {
            ...
            hallId: s.hallId,
            isActive: s.isActive,
            position: locationMeta
              ? { x: locationMeta.x, y: locationMeta.y }
              : null,
          };
        });

        const hallsWithoutMarkers = hallsWithMarkers.filter(
          (h) => !h.mapMarker && h.code !== "OUT",
        );
        const stationsWithoutPosition = stationsWithPositions.filter(
          (s) => !s.position,
        );
    ...
    try {
          ...
          stationQuery = db
            ...
            .where(
              and(
                ...
                or(
                  ilike(stations.code, searchPattern),
      ...
      if (!entityType || entityType === "STAND") {
        ...
        if (searchPattern) {
          standQuery = db
            ...
            .where(
              and(
                eq(stands.isActive, true),
                ilike(stands.identifier, searchPattern),
              ),
            );
        }
        ...
        for (const st of standsResult) {
          ...
          const material = st.materialId
            ? materialMap.get(st.materialId)
            : null;
          results.push({
            id: st.id,
            title: `Stellplatz ${st.identifier}`,
            subtitle: station
              ? `Station: ${station.name}${material ? ` | Material: ${material.name}` : ""}`
              : null,
    ...
    async (req, res) => {
      try {
        ...
        if (!confirm) {
          return res
            .status(400)
            .json({ error: "confirm: true is required to regenerate QR code" });
        }
