...
var quantityUnitEnum = pgEnum("quantity_unit", [
  "kg",
  "t",
  "m3",
  "pcs"
...
var customerContainersRelations = relations(
  customerContainers,
  ({ one, many }) => ({
...
var taskSchedulesRelations = relations(
  taskSchedules,
  ({ one, many }) => ({
    stand: one(stands, {
...
function getTodayBerlin() {
  const berlinDateStr = (/* @__PURE__ */ new Date()).toLocaleString("en-US", {
...
async function generateDailyTasksScheduled() {
  try {
    ...
    const openDailyTasks = await db.select().from(tasks).where(and2(eq2(tasks.taskType, "DAILY_FULL"), eq2(tasks.status, "OPEN")));
    ...
    const dailyFullStands = await db.select().from(stands).where(and2(eq2(stands.dailyFull, true), eq2(stands.isActive, true)));
    let createdCount = 0;
    let skippedCount = 0;
...
async function registerRoutes(app2) {
  ...
  app2.post("/api/admin/tasks", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      if (error?.code === "23505") {
        return res.status(409).json({
          error: "A task with this title already exists for this stand and date"
        });
      }
  ...
  app2.post("/api/tasks/:id/accept", async (req, res) => {
    try {
      ...
      if (LATER_STATES.includes(task.status)) {
        const sourceContainer2 = await storage.getCustomerContainer(
          task.containerID
        );
        let targetContainer2 = null;
        if (task.deliveryContainerID) {
          targetContainer2 = await storage.getWarehouseContainer(
      ...
      if (!updatedTask) {
        return res.status(400).json({
          error: "Ung\xFCltiger Status-\xDCbergang. Aktueller Status: " + task.status
        });
      }
      ...
      if (task.claimedByUserId) {
        if (!isClaimExpired(task.claimedAt)) {
          ...
          return res.status(409).json({
            ...
            expiresAt: task.claimedAt ? new Date(
              task.claimedAt.getTime() + CLAIM_TTL_MINUTES * 60 * 1e3
            ) : null
          });
        } else {
  ...
  app2.post("/api/scan-events", async (req, res) => {
    try {
      ...
      if (scanContext === "TASK_COMPLETE_AT_WAREHOUSE") {
        if (!taskId) {
          return res.status(400).json({
            error: "taskId ist erforderlich f\xFCr Lager-Abschluss-Scan"
          });
        }
        if (measuredWeight === void 0 || measuredWeight === null) {
          return res.status(400).json({
            error: "measuredWeight ist erforderlich f\xFCr Lager-Abschluss-Scan"
          });
        }
  ...
  app2.get("/api/activity-logs/export/csv", async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=aktivitaetslog-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv`
      );
      res.send("\uFEFF" + csv);
    } catch (error) {
    ...
    try {
      ...
      const driverStats = drivers.map((driver) => {
        ...
        return {
          ...
          inProgress: driverTasks.filter(
            (t) => inProgressStatuses.includes(t.status)
          ).length,
          completionRate,
      ...
      const overallStats = {
        ...
        totalCompletedToday: driverStats.reduce(
          (sum2, d) => sum2 + d.completedToday,
          0
        ),
        totalCompletedThisWeek: driverStats.reduce(
          (sum2, d) => sum2 + d.completedThisWeek,
          0
        ),
        avgCompletionRate: driverStats.length > 0 ? Math.round(
          driverStats.reduce((sum2, d) => sum2 + d.completionRate, 0) / driverStats.length
        ) : 0
      };
      res.json({
        drivers: driverStats,
  ...
  app2.get("/api/analytics/fill-trends", async (req, res) => {
    try {
      ...
      const materialBreakdown = warehouseContainers2.reduce(
        (acc, c) => {
  ...
  app2.get("/api/dashboard/stats", async (req, res) => {
    try {
      ...
      const openTasks = tasksToCount.filter(
        (t) => openStatuses.includes(t.status)
      ).length;
      ...
      const cancelledTasks = tasksToCount.filter(
        (t) => cancelledStatuses.includes(t.status)
      ).length;
      ...
      try {
        ...
        const driverOverview = await Promise.all(
          drivers.map(async (driver) => {
            ...
            const activityLogs2 = await storage.getActivityLogs({
              userId: driver.id
            });
            const lastActivity = activityLogs2.length > 0 ? activityLogs2[0].timestamp : null;
            return {
              ...
              openTasks: driverTasks.filter(
                (t) => openStatuses.includes(t.status)
              ).length,
              inProgressTasks: driverTasks.filter(
                (t) => inProgressStatuses.includes(t.status)
              ).length,
              completedTasks: driverTasks.filter(
                (t) => completedStatuses.includes(t.status)
              ).length,
              cancelledTasks: driverTasks.filter(
                (t) => cancelledStatuses.includes(t.status)
              ).length,
  ...
  app2.post("/api/stations", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      const [existingStation] = await db.select().from(stations).where(and2(eq2(stations.hallId, hallId), eq2(stations.code, code)));
      if (existingStation) {
        return res.status(400).json({
  ...
  app2.patch(
    ...
    async (req, res) => {
      try {
        ...
        const [duplicateStation] = await db.select().from(stations).where(
          and2(
            eq2(stations.hallId, hallId),
            eq2(stations.code, existingStation.code)
          )
        );
        if (duplicateStation) {
          return res.status(400).json({
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        ...
        const result = await db.select().from(boxes).where(and2(eq2(boxes.standId, standId), eq2(boxes.isActive, true)));
        res.json(result);
      } catch (error) {
  ...
  app2.put("/api/automotive/tasks/:id/status", requireAuth, async (req, res) => {
    try {
      ...
      if (scannedBoxId && task.boxId && scannedBoxId !== task.boxId) {
        return res.status(400).json({
          error: "Gescannte Box stimmt nicht mit der Aufgabe \xFCberein"
        });
      }
  ...
  app2.get("/api/daily-tasks/today", async (req, res) => {
    try {
      const todayStr = formatDateBerlin(/* @__PURE__ */ new Date());
      const todayTasks = await db.select().from(tasks).where(and2(eq2(tasks.taskType, "DAILY_FULL"), eq2(tasks.status, "OPEN")));
  ...
  app2.post("/api/scan/place-box", requireAuth, async (req, res) => {
    try {
      ...
      const standMeta = await buildStandContextMeta(stand.id);
      const existingBoxes = await db.select().from(boxes).where(and2(eq2(boxes.standId, stand.id), eq2(boxes.status, "AT_STAND")));
  ...
  app2.get("/api/activity", async (req, res) => {
    try {
      ...
      const limit = Math.min(
        100,
        Math.max(1, parseInt(limitParam) || 50)
      );
      ...
      if (materialId && typeof materialId === "string") {
        conditions.push(
          sql3`${taskEvents.metaJson}->>'materialId' = ${materialId}`
        );
      }
      if (stationId && typeof stationId === "string") {
        conditions.push(
          sql3`${taskEvents.metaJson}->>'stationId' = ${stationId}`
        );
      }
      if (hallId && typeof hallId === "string") {
      ...
      if (groupBy === "material") {
        const result = await db.select({
          ...
          avgLeadTimeMinutes: avg(
            sql3`EXTRACT(EPOCH FROM (${tasks.disposedAt} - ${tasks.createdAt})) / 60`
          )
        ...
        if (groupBy === "day") {
          ...
          return res.status(400).json({
            error: "Invalid groupBy parameter. Use: material, day, week, or month"
          });
        }
        const result = await db.select({
          period: dateExpr,
  ...
  app2.get("/api/analytics/backlog", requireAuth, async (req, res) => {
    try {
      ...
      const tasksWithHours = result.map((task) => ({
        ...task,
        hoursInStatus: task.updatedAt ? Math.round(
          (now - new Date(task.updatedAt).getTime()) / (1e3 * 60 * 60)
        ) : null
      }));
  ...
  app2.get(
    ...
    async (req, res) => {
      try {
        ...
        }).from(stands).leftJoin(materials, eq2(stands.materialId, materials.id)).where(
          and2(
            eq2(stands.stationId, stationId),
...
function setupCors(app2) {
  app2.use((req, res, next) => {
    ...
    if (origin) {
      ...
      const isAllowed = allowedPatterns.some((pattern) => pattern.test(origin)) || process.env.EXPO_PUBLIC_DOMAIN && origin.includes(process.env.EXPO_PUBLIC_DOMAIN);
      if (isAllowed) {
